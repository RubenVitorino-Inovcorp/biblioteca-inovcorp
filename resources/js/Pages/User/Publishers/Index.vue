...
<template>
    <AppLayout title="Catálogo de Editoras">
        ...
        <div v-if="publishers.data.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 xl:mx-10 gap-6 p-4">
            <div v-for="publisher in publishers.data" :key="publisher.id" class="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] hover:shadow-[0_8px_20px_-6px_rgba(6,81,237,0.15)] hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col group">
                <Link :href="route('catalog.editoras.show', publisher.id)" class="block flex-grow">
                    ...
                    <div class="p-5 flex flex-col items-center text-center border-t border-gray-50">
                        <h2 class="font-bold text-lg text-[#191c1e] truncate w-full group-hover:text-[#006c49] transition-colors" :title="publisher.name">
                            {{ publisher.name }}
                        </h2>
